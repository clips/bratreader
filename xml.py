...
def importxml(filename):
    ...
    for s in sentences.getchildren():
        ...
        sentobjects.append(Sentence(key=s.get('id').split(".")[1], line=repr, start=int(s.get("start"))))

    for annotation in annotations.getchildren():

        id = unicode(annotation.get('id')[3:])
        repr = unicode(annotation.get('repr'))
        spans = [[int(y) for y in x.split("|")]for x in annotation.get('spans').split(",")]
    ...
    for annotation in annotations.getchildren():
        ...
        for x, y in {unicode(x): unicode(y) for x, y in annotation.attrib.items() if x not in ["id", "repr", "spans", "words"]}.items():
